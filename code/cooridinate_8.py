...
# #起始点、目标点以及SVO
goal1=np.array([200, 330])
goal3=np.array([350,190])

goal2=np.array([190, 50])
goal4=np.array([50, 190])

goal5=np.array([290,310])
goal6=np.array([290,90])

goal7=np.array([90,90])
goal8=np.array([90,310])

origin1 = np.array([200, 50])
origin3 = np.array([50, 190])

origin2 = np.array([190, 330])
origin4 = np.array([350,190])

origin5=np.array([90,90])
origin6=np.array([90,310])

origin7=np.array([290,310])
origin8=np.array([290,90])


svo=np.array([0,0,0,0,0,0,0,0])
...
class Maze(tk.Tk, object):
    ...
    def resetRobot(self):
        ...
        self.rect5 = round_rectangle(origin5[0] - 5, origin5[1] - 5,
                                     origin5[0] + 15, origin5[1] + 15,
                                     fill='PaleGreen', radius=3, outline='black')
        self.rect6 = round_rectangle(origin6[0] - 5, origin6[1] - 5,
                                     origin6[0] + 15, origin6[1] + 15,
                                     fill='Green', radius=3, outline='black')
        self.rect7 = round_rectangle(origin7[0] - 5, origin7[1] - 5,
                                     origin7[0] + 15, origin7[1] + 15,
                                     fill='OliveDrab', radius=3, outline='black')
        self.rect8 = round_rectangle(origin8[0] - 5, origin8[1] - 5,
                                     origin8[0] + 15, origin8[1] + 15,
                                     fill='DarkGreen', radius=3, outline='black')
...
def  nash_learn(state):#通过casadi找最优控制
    ...
    position = ca.SX.sym('po', n_state * n_player)#共16个代表8个智能体的状态
    action = ca.SX.sym('act', n_control * n_player)#共16个代表对于智能体的控制作用，即，速度
    ...
    for inter3 in range(1,4):
        ...
        lbg.append(0)
        ubg.append(np.inf)
        g.append(ca.norm_2((x[6:8, inter3] + interval) - (env.obstacle + interval2))-70)
        lbg.append(0)
        ubg.append(np.inf)
        g.append(ca.norm_2((x[8:10, inter3] + interval) - (env.obstacle + interval2))-70)
        lbg.append(0)
        ubg.append(np.inf)
        g.append(ca.norm_2((x[10:12, inter3] + interval) - (env.obstacle + interval2))-70)
        lbg.append(0)
        ubg.append(np.inf)
        g.append(ca.norm_2((x[12:14, inter3] + interval) - (env.obstacle + interval2))-70)
        lbg.append(0)
        ubg.append(np.inf)
        g.append(ca.norm_2((x[14:16, inter3] + interval) - (env.obstacle + interval2))-70)
        lbg.append(0)
        ubg.append(np.inf)
