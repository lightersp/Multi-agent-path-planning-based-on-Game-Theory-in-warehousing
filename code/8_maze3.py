...
MAZE_H = 21  # 格子的高
MAZE_W = 21  # 格子的宽
writer=SummaryWriter('8_maze3')
#起始点、目标点以及SVO
goal1=np.array([19 * UNIT, 9 * UNIT])
goal2=np.array([19 * UNIT, 12 * UNIT])
goal3=np.array([9 * UNIT, 18 * UNIT])
goal4=np.array([11 * UNIT, 18 * UNIT])
goal5=np.array([1 * UNIT, 9 * UNIT])
goal6=np.array([1 * UNIT, 12 * UNIT])
goal7=np.array([9 * UNIT, 3 * UNIT])
goal8=np.array([11 * UNIT, 3 * UNIT])
origin1 = np.array([190, 70])
origin2 = np.array([230, 70])
origin3 = np.array([390, 190])
origin4 = np.array([390,250])
origin5 = np.array([190,370])
origin6 = np.array([230,370])
origin7 = np.array([30,190])
origin8 = np.array([30,250])
...
def  nash_learn(state):#通过casadi找最优控制
    ...
    for inter3 in range(1,N+1):
        ...
        for inter4 in range(len(env.obstacle)):
            g.append(ca.norm_2((x[0:2, inter3] + interval) - (env.obstacle[inter4] + interval))-25)
            lbg.append(0)
            ubg.append(np.inf)
            g.append(ca.norm_2((x[2:4, inter3] + interval) - (env.obstacle[inter4] + interval))-25)
            lbg.append(0)
            ubg.append(np.inf)
            g.append(ca.norm_2((x[4:6, inter3] + interval) - (env.obstacle[inter4] + interval))-25)
            lbg.append(0)
            ubg.append(np.inf)
            g.append(ca.norm_2((x[6:8, inter3] + interval) - (env.obstacle[inter4] + interval))-25)
            lbg.append(0)
            ubg.append(np.inf)
            g.append(ca.norm_2((x[8:10, inter3] + interval) - (env.obstacle[inter4] + interval))-25)
            lbg.append(0)
            ubg.append(np.inf)
            g.append(ca.norm_2((x[10:12, inter3] + interval) - (env.obstacle[inter4] + interval))-25)
            lbg.append(0)
            ubg.append(np.inf)
            g.append(ca.norm_2((x[12:14, inter3] + interval) - (env.obstacle[inter4] + interval))-25)
            lbg.append(0)
            ubg.append(np.inf)
            g.append(ca.norm_2((x[14:16, inter3] + interval) - (env.obstacle[inter4] + interval))-25)
            lbg.append(0)
            ubg.append(np.inf)
    lbx=-3
    ubx=3
