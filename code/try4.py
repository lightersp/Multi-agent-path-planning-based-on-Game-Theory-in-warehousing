...
writer=SummaryWriter('svo3pi/8')
# logFile = 'log__selfish.txt'  # 记录数据
# fd = open(logFile, 'w')
# s = 'agent1=%d,agent2=%d,agent3=%d,agent4%d' % (0, 0, 0, 0)
# fd.write(s + '\n')
# fd.close()
svo=np.array([np.pi/9,np.pi/9,np.pi/9,np.pi/9])
...
class Maze(tk.Tk, object):
    ...
    def _build_maze(self):
        ...
        for count in range(37):
            s1=eval(f.readline())
            s2=eval(f.readline())
            self.canvas.create_line(s1[0][0] + 10, s1[0][1] + 10, s2[0][0] + 10, s2[0][1] + 10, fill='SkyBlue1',width=4)
            self.canvas.create_line(s1[1][0] + 10, s1[1][1] + 10, s2[1][0] + 10, s2[1][1] + 10,fill='SteelBlue2',width=4)
            self.canvas.create_line(s1[2][0] + 10, s1[2][1] + 10, s2[2][0] + 10, s2[2][1] + 10, fill='RoyalBlue1', width=4)
            self.canvas.create_line(s1[3][0] + 10, s1[3][1] + 10, s2[3][0] + 10, s2[3][1] + 10, fill='Midnightblue',width=4)
        #画路径点 sv0=0
        # logFile = 'log__altruistic.txt'  # 记录数据
        # f = open(logFile, 'r')
        # s = f.readline()
...
def move(action):
    ...
    # s='{},{},{},{}'.format(state[:2],state[2:4],state[4:6],state[6:])#记录步骤
    # fd = open(logFile, 'a')
    # fd.write(s + '\n')
    # fd.close()
